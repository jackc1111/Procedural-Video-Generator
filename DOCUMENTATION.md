...
```bash
python src/generator.py [path_to_script.py] [ClassName] --quality [low|medium|high|4k] [--studio-format] [--composite-video path/to/video.mp4]
```
...
- **--composite-video**: Шлях до відео файлу для композитингу поверх анімації з використанням MoviePy. Результат з суфіксом `_composite.mp4`.
